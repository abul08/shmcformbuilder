              ...
              {forms?.map((form) => (
                <Link key={form.id} href={`/f/${form.slug}`} className="block group">
                  <div className="bg-white/5 border border-white/10 rounded-xl p-6 hover:bg-white/10 hover:border-primary/50 transition-all duration-300 h-full flex flex-col">
                    <h3 className="text-xl font-bold text-white mb-2 group-hover:text-primary transition-colors">{form.title}</h3>
                    <p className="text-gray-400 text-sm mb-4 line-clamp-2 flex-grow">{form.description || 'No description provided.'}</p>
